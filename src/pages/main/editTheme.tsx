  ...
  const getTheme = async () => {
    try {
      ...
      const res = await fetch(
        `https://scano-0df0b7c835bf.herokuapp.com/api/v1/themes/${id}`,
        {
      ...
      if (res.ok) {
        ...
        console.log(data);
        const foundThemeType = options.findIndex(option => option.key === data.theme_type);
        const foundSrc = src.findIndex(src => src.key === data.source_types[0]);
        const foundLang = lang.findIndex(lang => lang.key === data.language);
        setThemeName(data.name);
        setSelectedOption(options[foundThemeType]);
        setSearchWords(data.keywords.join(' '));
        setUnsearchWords(data.minus_keywords.join(' '));
        setSelectSrc(src[foundSrc]);
        setSearchRegionSelect(data.search_domains);
        setMaterialTypesSelect(data.material_types);
        setSrcLang(lang[foundLang]);
        setUnsearchSrc(data.exclude_sources.join(' '));
        setPending(false);
      } else {
        setPending(false);
      }
      ...
      <MainLayout>
        <div className="pb-10">
          <div className="p-4 bg-white rounded-lg w-2/3">
            ...
            } size="sm" classBtn="!w-fit mt-8" onClick={updateTheme} />
          </div>
        </div>
      </MainLayout>
    </ProtectLayout>
